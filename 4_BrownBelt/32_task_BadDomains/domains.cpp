...
private:
	...
	void ParseFullDomains(const vector<string>& raw_domains) {
		for (const auto& domain : raw_domains) {
			vector<string> subdomains = SplitBy(domain);
			reverse(subdomains.begin(), subdomains.end());

			auto* curr_line = &bad_domains_.data_;
			for (auto& dommain : subdomains) {
				curr_line->insert({dommain, make_shared<Item>()});
				curr_line = &curr_line->at(dommain)->data_;
			}
			curr_line->insert({"", make_shared<Item>()});
		}
	}

	vector<string> SplitBy(const string& s, char split = '.') {
		vector<string> result;
		auto curr = s.begin();

		while (curr != s.end()) {
			auto space = find(curr, s.end(), split);
...
int main() {
	vector<string> bad_domains = ReadFullDomains(cin);
	vector<string> domains = ReadFullDomains(cin);
