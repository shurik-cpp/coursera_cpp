...
public:
	...
	friend ostream& operator<<(ostream& output, const HttpResponse& response);
};
//-----------------------------------------------
ostream& operator<<(ostream& output, const HttpResponse& response) {
	output << "HTTP/1.1 " << static_cast<int>(response.http_code);
	...
	if (response.body.size() > 0) {
		...
		output << '\n'; // <-эта строчка даже не нужна была )
	}
