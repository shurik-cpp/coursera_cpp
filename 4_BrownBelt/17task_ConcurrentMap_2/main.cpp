...
public:
	...
	WriteAccess operator[](const K& key) {
		...
		return {lock_guard(lockers[index]), data[index][key]};
	}
	...
	ReadAccess At(const K& key) const {
		...
		const size_t index = GetIndex(key);
		return {lock_guard(lockers[index]), data[index].at(key)};
	}

	bool Has(const K& key) const {
		const size_t index = GetIndex(key);
		lock_guard guard(lockers[index]);
		if (data.at(index).count(key)) {
			return true;
		}
		return false;
	}
	...
	MapType BuildOrdinaryMap() const {
		...
		for (const auto& it : data) {
			lock_guard guard(lockers[locker]);
			result.insert(begin(it), end(it));
			locker++;
		}
