...
struct AddressHasher {
	...
	size_t operator()(const Address &a) const {
		size_t c = shash(a.city);
		size_t s = shash(a.street);
		const size_t x = 2'946'901;
		return (c * x * x + s * x + a.building);
	}
};
...
struct PersonHasher {
	...
	size_t operator()(const Person &p) const {
		size_t n = shash(p.name);
		size_t h = p.height;
		size_t w = dhash(p.weight);
		size_t a = ahash(p.address);
		const size_t x = 2'946'901;
		return (n * x * 2 * x + h * x * x + w * x + a);
	}
};
