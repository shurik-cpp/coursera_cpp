...
public:
	...
	// метод AddShape() добавляет новую фигуру с помощью функции MakeShape(), которую вам нужно реализовать.метод AddShape()
	// добавляет новую фигуру с помощью функции MakeShape(), которую вам нужно реализовать.
	ShapeId AddShape(ShapeType shape_type, Point position, Size size,
									 unique_ptr<ITexture> texture) {
		unique_ptr<IShape> shape = MakeShape(shape_type); // создается форма shape_type на холсте shape
	...
	// метод DuplicateShape() клонирует существующую фигуру, вызывая метод Clone(),
	// который вам, опять же, нужно будет реализовать.метод DuplicateShape() клонирует существующую фигуру,
...
private:
	...
	Size size_ = {};
	ShapeId current_id_ = 0; // using ShapeId = size_t;
	Shapes shapes_; // using Shapes = map<ShapeId, unique_ptr<IShape>>;
};

void TestSimple() {
	Canvas canvas; // создаем холст (canvas)
	canvas.SetSize({5, 3}); // задаем размер холста

	canvas.AddShape(ShapeType::Rectangle, {1, 0}, {3, 3}, nullptr); // добавляем фигуру (форму - shape)
